...
bool is_cycle(int original, int locked_as_loser);
...
// Check if locking pair creates a cycle
bool is_cycle(int original, int locked_as_loser)
{
    if (locked_as_loser == original)
    {
        return true;
    }
    for (int i = 0; i < candidate_count; i++)
    {
        if (locked[locked_as_loser][i] && is_cycle(original, i))
        {
            return true;
        }
